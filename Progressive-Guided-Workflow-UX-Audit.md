# CountIn Progressive Guided Workflow — UX Audit
...
CountIn has many **long, multi-section screens** where the final action (Save, Share, Create, Submit, Continue) appears before the owner has naturally reviewed later sections. Menu Planning exposed this clearly: owners selected meals and tapped Share before noticing Extras.
...
## CountIn Progressive Guided Workflow (shared UX pattern)
...
1. **Adopt Progressive Guided Workflow as a CountIn-wide UX pattern**, documented above, with meals as the reference implementation.  
...
Storage: `@countin/coachmarks/{spaceId}/{tourId}`. Gate: NEW / SETUP_IN_PROGRESS only.
